...
class GenerateVPEmail(MailCommonCode):
    ...
    def get_json_file(self, dsru_version):
        ...
        url = os.path.join(path, "VPSecurityUpdate{}.json".format(dsru_version.replace("-", "")))
        print("# dsru_file: {}".format(url))
        return requests.get(url, headers={'Authorization': 'Bearer '+self.token}).json()

    # this is main method to generate vp mail
    def geneare_vp_mail(self, dsru_flag):
